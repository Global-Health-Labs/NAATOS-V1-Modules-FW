...
naatos_gpregret2_t gpregret2;  // track the general purpose retained register 2 (1 is used for DFU entry)
...
void set_startup_enables(void) {
  ...
  NRF_GPIO->PIN_CNF[MOTOR_BRAKE_N_PIN] |= (NRF_GPIO_PIN_H0H1 << GPIO_PIN_CNF_DRIVE_Pos); 
...
void create_tasks() {
  ...
  #endif
  //comrx Task
  xReturned = xTaskCreate(serialrx_task, "COMRX", 1024, NULL, 0, &comrxTaskHandle);
  if (xReturned != pdPASS) {
    // The task was created.  Use the task's handle to delete the task.
    sprintf(buff, "Error creating SerialRX task. Error: %d", xReturned);
    send_debug_log_message(buff);
