  ...
  while (runThrough) {
    ...
    case CYCLE_2_TIMER: {
      if (last_state != current_state) {
        start_time = xTaskGetTickCount();
        if (use_default_configuration_parameters) {
          end_time = ((DEFAULT_CYCLE_3_RUNTIME)*configTICK_RATE_HZ);
        } else {
          end_time = ((config.cycle_3_run_time_s) * configTICK_RATE_HZ);
        }
      }
