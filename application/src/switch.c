...
void vButtonTimerCallback(TimerHandle_t xTimer) {
  ...
  if (xReturned != pdPASS) {
    send_debug_log_message("SWITCH_TASK: Unable to send timer update to buttonRxQueue queue.");
  }
}
...
void startButtonTimer(void) {
  ...
  if (xTimerChangePeriod(buttonTimer, sampleRateTicks, 100) != pdPASS) {
    send_debug_log_message("SWITCH_TASK: Cannot change period of button timer. ");
  }

  if (xTimerStart(buttonTimer, 0) != pdPASS) {
    send_debug_log_message("SWITCH_TASK: Failed to start button timer. ");
  }
}

void stopButtonTimer(void) {
  if (xTimerStop(buttonTimer, 100) != pdPASS) {
    send_debug_log_message("Failed to stop button timer. ");
  }
}
...
void buttonTask(void *pvParameters) {
  ...
  while (true) {
    ...
    if (xReturned != pdPASS) {
      send_debug_log_message("Unable to Rx data to button queue");
    } else {
      ...
      case BUTTON_MSG_WAKE:
        ...
        if (notify <= 0) {
          ...
          if (xReturned != pdPASS) {
            send_debug_log_message("SWITCH_TASK: Unable to send tasks wake up to main_wakeupTasksQueue.");
          }
          notify++;
        }
        break;
